...
export const bracketRoutes = new Elysia({ prefix: '/api/v1/tournaments/:id' })
  .use(authPlugin)
  // Public routes
  .get('/bracket', async ({ params }) => {
    // 查询 1: tournament + stages（JOIN 合并，减少网络往返）
    ...
    // 查询 2: matches + games + teams（JOIN 合并，减少网络往返）
    ...
    // 按 matchId 聚合，处理 LEFT JOIN 产生的笛卡尔积
    ...
    return { tournament, stages: bracketStages };
  })
  .get('/stages', async ({ params }) => {
    return db.select().from(stages).where(eq(stages.tournamentId, params.id));
  })
  .get('/matches', async ({ params }) => {
    // 单次 JOIN 查询 stages + matches + teams，减少网络往返
    const t1 = alias(teams, 't1');
    const t2 = alias(teams, 't2');
    const rows = await db.select({
      ...
      .leftJoin(t1, eq(t1.id, matches.team1Id))
      .leftJoin(t2, eq(t2.id, matches.team2Id))
      .where(eq(stages.tournamentId, params.id));
  ...
  .get('/stages/:stageId/matches', async ({ params }) => {
    return db.select().from(matches).where(eq(matches.stageId, params.stageId));
  })
  .get('/standings', async ({ params }) => {
    // 单次 JOIN 查询 stages + standings + teams，减少网络往返
    const rows = await db.select({
      rank: standings.rank,
  ...
  .get('/standings/:groupLabel', async ({ params }) => {
    // 单次 JOIN 查询，减少网络往返
    const rows = await db.select({
      rank: standings.rank,
      wins: standings.wins,
      ...
      .innerJoin(stages, eq(stages.id, standings.stageId))
      .leftJoin(teams, eq(teams.id, standings.teamId))
      .where(and(eq(stages.tournamentId, params.id), eq(standings.groupLabel, params.groupLabel)));
  ...
  // Admin routes
  .use(requireAdmin)
  .post('/reset', async ({ params }) => {
    // 重置赛程：删除所有 stages + matches + games + standings，赛事回到 draft 状态
    ...
    await db.update(tournaments).set({ status: 'draft' }).where(eq(tournaments.id, params.id));
    return { message: '赛程已重置' };
  })
  .post('/generate', async ({ params, body, user }) => {
    const [tournament] = await db.select().from(tournaments).where(eq(tournaments.id, params.id)).limit(1);
    if (!tournament) throw new AppError('NOT_FOUND', '赛事不存在', 404);
    ...
    if (data.seed_by === 'manual' && data.seed_order) {
      seededTeams = data.seed_order
        .map((id, i) => {
          const t = tournamentTeamsList.find((tt) => tt.id === id);
          if (t) t.seed = i + 1;
          return t;
        })
    ...
    // Insert matches (two-pass for nextMatchId)
    ...
    // Second pass: update nextMatchId
    for (let i = 0; i < result.matches.length; i++) {
      const match = result.matches[i];
    ...
    // Third pass: 推进 walkthrough（轮空）比赛的 winner 到 nextMatch
    // 轮空比赛的 winnerId 在生成时已设置，但 nextMatch 的 team1Id/team2Id 为 null
    // 需要手动填充，否则下一轮会显示 TBD vs TBD
    // 循环处理以应对连续轮空（A 轮空 → 推进到 B → B 也轮空 → 推进到 C）
    ...
    while (walkChanged) {
      ...
      for (const m of walkthroughMatches) {
        ...
        // 填充第一个空槽位（team1 或 team2），避免重复填充同一队伍
    ...
    // 循环赛/瑞士轮：初始化积分榜（每队 0 胜 0 负 0 积分）
    // 这样赛程生成后积分榜即可显示所有队伍，而非空数据
    if (tournament.format === 'round_robin' || tournament.format === 'swiss') {
      const mainStage = insertedStageIds[0]; // 主 stage
    ...
    await db.update(tournaments).set({ status: 'ongoing' }).where(eq(tournaments.id, params.id));

    return { message: '赛程生成成功', stagesCount: result.stages.length, matchesCount: result.matches.length };
  })
  .post('/generate-next-round', async ({ params }) => {
    const [tournament] = await db.select().from(tournaments).where(eq(tournaments.id, params.id)).limit(1);
    if (!tournament) throw new AppError('NOT_FOUND', '赛事不存在', 404);
    ...
    const tournamentStages = await db.select().from(stages).where(eq(stages.tournamentId, params.id));
    ...
    return { message: `第${currentRound + 1}轮生成成功`, matchesCount: newMatches.length };
  });
